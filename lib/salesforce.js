...
class Salesforce {
    ...
    async getJobData(jobId, locator = null) {
        ...
        try {
            ...
            //save to temp csv file since the libary only reads from files
            if (!fs.existsSync('/temp')) {
                fs.mkdirSync('/temp')
            }
            fs.writeFileSync(`/temp/${jobId}`, res.data, 'utf8');

            //load the csv file and convert to an array
            const jsonData = csvToJson.formatValueByType().parseSubArray(",", '"').getJsonFromCsv(`/temp/${jobId}`);

            //delete the temp file
            fs.unlinkSync(`/temp/${jobId}`);
