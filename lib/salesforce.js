...
class Salesforce {
    ...
    //bulk load data
    #bulkLoad(salesforceObject, externalId, data) {
        return new Promise((resolve) => {
            let jobId;
            const job = this.#sfConnection.bulk.load(salesforceObject, 'upsert', {extIdField: externalId}, data);
    ...
    async bulkUpsertTable(salesforceObject, objectData, externalId, callback) {
        try {
            ...
            const jobPromises = [];
            for (const i of d) {
                jobPromises.push(this.#bulkLoad(salesforceObject, externalId, i));
            }
