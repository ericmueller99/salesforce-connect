...
class Salesforce {
    ...
    async insertSingleRecord(objectName, record, callback) {

        if (!objectName || !record) {
            return callback ? callback(new Error('objectName and record are required')) : Promise.reject(new Error('objectName and record are required'));
        }

        await this.authenticate();

        this.#sfConnection.sobject.create(objectName, record, (error, res) => {

            if (error) {
                return callback ? callback(error) : Promise.reject(error);
            }

            const failures = res.filter(e => !e.success);
            if (failures.length > 0) {
                return callback ? callback(new Error(`Record failed to insert.  Error was ${JSON.stringify(failures)}`)) : Promise.reject(new Error(`Record failed to insert.  Error was ${JSON.stringify(failures)}`));
            }
