...
class Salesforce {
    ...
    async getJobData(jobId, locator = null) {
        ...
        //making sure the temp directory exists to save the csv files in.
        ...
        try {
            ...
            fs.writeFileSync(path.join(__dirname, `../temp/${locator && locator !== null ? locator : 'last'}.csv`), res.data, 'utf8');
            ...
            const processData = () => {
                return new Promise(resolve => {
                    ...
                    let csvData = [];
                    fs.createReadStream(path.join(__dirname, `../temp/${locator && locator !== null ? locator : 'last'}.csv`))
                    .pipe(csv())
                    .on('data', data => {
                        csvData.push(data);
                    })
                    .on('end', () => {
                        fs.unlinkSync(path.join(__dirname, `../temp/${locator ? locator : 'last'}.csv`));
                        resolve(csvData)
                    });
