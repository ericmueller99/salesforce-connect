    ...
    async getLeadOrContact(emailAddress, callback = null) {
        try {
            ...
            if (contacts.length === 1) {
                ...
                //TODO figure out which is the best contact to use.
            }
            else if (leads.length === 0) {
                leadOrContact = leads[0]
            }
            else if (leads.length > 1) {
                //TODO figure out which lead is best to use.
            }
            else {
                return callback ? callback(null, []) : resolve([]);
            }

            const prefQuery = `SELECT Id from Preferences__c where Contact_Name__c=:${leadOrContact.Id} or Lead_Name__c=:${leadOrContact.Id}`;
            const preferences = await this.#search(prefQuery);
